// AddStudentDrawer.jsx - Drawer Component for Adding/Editing Students
...
import {
  ...
  Phone, BookOpen, FolderOpen, Eye, Check, Settings, ArrowLeft, UserPlus
} from "lucide-react";

// Drawer accepts: { open, onClose, onSuccess, classes, initialClass, initialStudentId, initialMode }
...
export default function AddStudentDrawer({ open, onClose, onSuccess, classes = [], initialClass = '', initialStudentId = '', initialMode = 'add' }) {
  const mode = initialMode;
  const classFromUrl = initialClass;
  const studentIdFromUrl = initialStudentId;
  ...
  return (
    <>
      ...
      >
        <div className="flex flex-col h-full" style={{ background: 'var(--bg-secondary, #0f172a)', borderLeft: '1px solid rgba(255,255,255,0.08)' }}>
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-white/8 flex-shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="font-bold text-white text-sm">{mode === 'edit' ? 'Edit Student' : 'New Admission'}</h2>
                <p className="text-xs text-slate-500">{studentData ? `ID: ${studentData.studentId}` : 'Fill in student details'}</p>
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-5">
            ...
            {/* School ID info bar */}
            <div className="glass-card p-3 flex items-center justify-between mb-1">
              <span className="text-xs text-slate-400">School ID: <span className="text-indigo-400 font-mono font-semibold">{schoolId}</span></span>
              <span className="text-xs text-emerald-400">{schoolIdSource === 'localStorage' ? '✓ From storage' : '⚠ Default ID'}</span>
            </div>
            ...
            {showDialog && !apiError && (
              <div className="glass-card p-6 text-center space-y-4">
                ...
                <h3 className="font-bold text-white">Admission Workflow</h3>

                <div className="glass-card p-4 text-left">
                  <p className="text-xs text-slate-400 mb-3 flex items-center gap-1"><User size={13} className="text-indigo-400" /> New Enrollment</p>
                  <select value={selectedClass} onChange={(e) => setSelectedClass(e.target.value)} className="input-dark mb-3 w-full">
                    <option value="">-- Select Class --</option>
                    {classes.length > 0
                      ? classes.map((c, i) => <option key={i} value={c}>{c}</option>)
                      : ['class-1', 'class-2', 'class-3', 'class-4', 'class-5', 'class-6', 'class-7', 'class-8', 'class-9', 'class-10', 'class-11', 'class-12'].map(c => <option key={c} value={c}>{c.replace('class-', 'Class ')}</option>)
                    }
                  </select>
                  <button disabled={!selectedClass || isLoading || !schoolId} onClick={handleContinueNew} className="btn-primary w-full justify-center">
                    {isLoading ? <Loader size={15} className="animate-spin" /> : <Plus size={15} />}
                    {isLoading ? 'Creating...' : 'Start New Admission'}
                  </button>
                </div>

                <div className="glass-card p-4 text-left">
                  <p className="text-xs text-slate-400 mb-3 flex items-center gap-1"><FolderOpen size={13} className="text-violet-400" /> Load Existing Student</p>
                  <input type="text" placeholder="Student ID (e.g. S000005)" value={loadStudentId} onChange={(e) => setLoadStudentId(e.target.value)} className="input-dark mb-3 w-full" />
                  <button disabled={!loadStudentId || isFetchingExisting || !schoolId} onClick={handleContinueExisting} className="btn-secondary w-full justify-center">
                    {isFetchingExisting ? <Loader size={15} className="animate-spin" /> : <Upload size={15} />}
                    {isFetchingExisting ? 'Loading...' : 'Load Student Data'}
                  </button>
                </div>
              </div>
            )}
                ...
                <div className="glass-card p-4">
                  <div className="flex items-center gap-3 mb-3">
                    <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center text-white font-bold text-lg">
                      {(studentData.studentId || 'S')[0]}
                    </div>
                    <div>
                      <p className="font-bold text-white">{studentData.studentId}</p>
                      <p className="text-xs text-slate-400">Class: <span className="text-indigo-400">{studentData.className}</span></p>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    {[['Roll No', studentData.rollNumber], ['Section', studentData.section], ['School', schoolId], ['Created', formatTimestamp(studentData.createdAt)]].map(([k, v]) => v ? (
                    ...
                    <div className="glass-card p-4">
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        {documents.map((doc, idx) => {
                          if (doc.type === "Aadhar Card") {
                            let borderColorClass = 'border-white/15 bg-white/5';
                            if (doc.status === 'uploaded') borderColorClass = 'border-emerald-500/40 bg-emerald-500/10';
                            else if (doc.status === 'partial') borderColorClass = 'border-amber-500/40 bg-amber-500/10';
                            else if (doc.front.status === 'error' || doc.back.status === 'error') borderColorClass = 'border-rose-500/40 bg-rose-500/10';

                            return (
                              <div
                                key={idx}
                                className={`flex flex-col items-center justify-between relative h-32 rounded-xl overflow-hidden transition-all duration-300 border cursor-pointer ${borderColorClass}`}
                              ...
                              >
                                <div className="flex flex-col justify-center items-center h-full w-full p-2 text-center">
                                  {doc.front.url ? (
                                    ...
                                    <Pencil size={20} className="text-indigo-400 mb-1" />
                                  )}
                                  <div className="text-xs mt-1 w-full text-center border-t border-white/10 pt-1">
                                    <p className={`font-semibold capitalize text-xs ${doc.front.status === 'uploaded' ? 'text-emerald-400' : 'text-slate-500'}`}>F: {doc.front.status}</p>
                                    <p className={`font-semibold capitalize text-xs ${doc.back.status === 'uploaded' ? 'text-emerald-400' : 'text-slate-500'}`}>B: {doc.back.status}</p>
                                  </div>
                                </div>
                                <p className="text-xs font-medium py-1 w-full text-center border-t border-white/10 text-slate-400 truncate px-1">{doc.type}</p>
                              </div>
                            );
                          }
                            ...
                            >
                              ...
                              <div className="flex flex-col justify-center items-center h-full w-full p-2">
                                {doc.status === 'uploading' ? (
                                  <div className="text-center"><Loader size={20} className="text-amber-400 animate-spin mb-2" /><p className="text-xs text-slate-400">Uploading...</p></div>
                                ) : doc.status === 'error' ? (
                                  <div className="text-center"><AlertTriangle size={20} className="text-rose-400 mb-2" /><p className="text-xs text-slate-400">Error</p></div>
                                ) : (doc.url || doc.status === 'metadata_found') ? (
                                  <div className="text-center w-full">
                                    {doc.preview ? (
                                      <img src={doc.preview} alt={doc.type} className="h-12 w-full object-contain mb-1" />
                                    ) : doc.url ? (
                                      <div className="h-12 flex items-center justify-center bg-white/10 w-full rounded-lg"><FileText size={20} className="text-slate-400" /></div>
                                    ) : (
                                      <div className="h-12 flex items-center justify-center bg-violet-500/10 w-full rounded-lg"><Pencil size={20} className="text-violet-400" /></div>
                                    )}
                                    <p className={`text-xs font-semibold mt-1 ${doc.url ? 'text-emerald-400' : 'text-violet-400'}`}>{doc.url ? '✓ Done' : 'Meta'}</p>
                                  </div>
                                ) : (
                                  <div className="text-center"><CloudUpload size={20} className="text-indigo-400 mb-1" /><span className="text-xs text-slate-500 mt-1">Upload</span></div>
                                )}
                              </div>
                              <p className="text-xs font-medium py-1 w-full text-center border-t border-white/10 text-slate-400 truncate px-1">{doc.type}</p>
                              {doc.url && (
                    ...
                    <div className="glass-card p-4">
                      <div className="flex justify-between items-center mb-4">
                        ...
                        <button
                          ...
                          className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs transition-all bg-indigo-500/15 text-indigo-300 hover:bg-indigo-500/25"
                          disabled={isSaving}
                        >
