...
import { 
  ...
  Upload, ExternalLink, ZoomIn, X as CloseIcon, Calendar as CalendarIcon
} from 'lucide-react';
...
const DocumentsSection = ({ documents, onPreviewImage }) => {
...
// Enhanced Attendance History with Full Data Display
const AttendanceHistoryDetailed = ({ attendanceHistory }) => {
  const [showAll, setShowAll] = useState(false);
  const [selectedView, setSelectedView] = useState('list'); // 'list' or 'detailed'
  
  const displayHistory = showAll ? attendanceHistory : attendanceHistory.slice(0, 8);
    ...
    <div className="bg-gradient-to-br from-sky-50 via-white to-rose-50 border-2 border-blue-200 rounded-xl shadow-xl p-6">
      ...
      {attendanceHistory.length > 8 && (
        <div className="mt-4 text-center">
          ...
          >
            {showAll ? 'Show Less' : `Show All (${attendanceHistory.length})`}
          </button>
        </div>
      )}
    </div>
...
// Month Calendar Component for Attendance (existing)
const AttendanceCalendar = ({ attendanceHistory }) => {
  ...
  const getAttendanceForDay = (day) => {
    ...
    return attendanceHistory.find(record => record.date === dayStr);
  };
    ...
    <div className="bg-gradient-to-br from-sky-50 via-white to-rose-50 border-2 border-blue-200 rounded-xl shadow-xl p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-4">
          <button onClick={() => navigateMonth(-1)} className="p-2 hover:bg-blue-100 rounded-lg transition-colors">
      ...
      <div className="grid grid-cols-7 gap-1">
        {days.map((day, index) => {
          ...
          const isPresent = attendance && attendance.action === 'present_marked';
          const isAbsent = attendance && attendance.action === 'absent_marked';
          ...
          return (
            <div
              ...
              title={attendance ? `${day.getDate()} - ${isPresent ? 'Present' : 'Absent'}` : day.getDate()}
            >
              {day.getDate()}
            </div>
          );
      ...
      {/* Legend */}
      <div className="flex items-center justify-center gap-6 mt-6 pt-4 border-t border-blue-200">
        <div className="flex items-center">
...
// Fees History Component (existing)
const FeesHistory = ({ feesHistory }) => {
...
// Complains Section Component (existing)
const ComplainsSection = ({ complains }) => {
...
// Exam History Component (existing)
...
// Attendance Pie Chart Component (existing)
        ...
        <div className="relative w-40 h-40">
          <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
            ...
            <circle cx="50" cy="50" r="45" fill="none" stroke="#10b981" strokeWidth="8" 
                    strokeDasharray={`${presentStroke} ${circumference}`} strokeDashoffset="0" 
                    className="transition-all duration-1000" strokeLinecap="round" />
            <circle cx="50" cy="50" r="45" fill="none" stroke="#ef4444" strokeWidth="8" 
                    strokeDasharray={`${absentStroke} ${circumference}`} strokeDashoffset={-presentStroke} 
                    className="transition-all duration-1000" strokeLinecap="round" />
          </svg>
...
// Awards Section Component (existing)
...
export default function Studentinfo() {
  ...
  const loadAttendanceHistory = async () => {
    ...
    try {
      const apiUrl = `${API_BASE_URL}/attendance/${schoolId}/student/${studentId}/history`;
      ...
      if (result.success && Array.isArray(result.data)) {
        ...
        const presentCount = sortedHistory.filter(record => record.action === 'present_marked').length;
        const absentCount = sortedHistory.filter(record => record.action === 'absent_marked').length;
        const totalCount = presentCount + absentCount;
        
        setAttendanceStats({ present: presentCount, absent: absentCount, total: totalCount });
        
        console.log(`📊 Attendance Stats - Present: ${presentCount}, Absent: ${absentCount}, Total: ${totalCount}`);
      } else {
