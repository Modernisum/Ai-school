...
import {
  ...
  Hash, Shield, UserCheck, GraduationCap, DollarSign, Star, Tag, UploadCloud
} from 'lucide-react';
...
// Derive the server root (e.g. http://localhost:8080) from the API base for serving static uploads
...
export default function AddStudentPage({ onSuccess, onBack, mode: propMode, studentId: propStudentId }) {
  ...
  return (
    <div className="min-h-full bg-gradient-to-br from-slate-900 via-slate-900 to-slate-800">
      {/* Header */}
      <div className="sticky top-0 z-20 backdrop-blur-md bg-slate-900/80 border-b border-white/[0.06] px-4 sm:px-6 py-3 sm:py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <div>
            <h1 className="text-lg font-bold text-white flex items-center gap-2">
              ...
              {mode === 'edit' ? 'Edit Student Profile' : 'Admission'}
            </h1>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          {mode !== 'edit' && (
            <button
              ...
              className="px-4 py-2 rounded-xl text-sm font-bold bg-white/5 border border-white/10 text-slate-300 hover:text-white hover:bg-white/10 transition-all flex items-center gap-2"
            >
              <UploadCloud size={16} />
              Import
            </button>
      ...
      <BulkImportModal 
        ...
        onImport={async (payload) => {
          const res = await fetch(`${API_BASE}/students/${schoolId}/students/bulk`, { 
            method: 'POST', 
            headers: { 'Content-Type': 'application/json' }, 
          ...
          setToast({ type: 'success', msg: `Imported ${data.data?.successful || 0} students!` });
          setBulkModalOpen(false);
          // Wait a bit then refresh list (via onSuccess callback)
