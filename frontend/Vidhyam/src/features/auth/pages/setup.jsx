...
const CLASS_LEVELS = [
  { label: "Primary (Up to Class 5)", value: 8, description: "Pre-Nursery to Class 5" },
  { label: "Junior (Up to Class 8)", value: 11, description: "Pre-Nursery to Class 8" },
  { label: "High School (Up to Class 10)", value: 13, description: "Pre-Nursery to Class 10" },
  { label: "Intermediate (Up to Class 12)", value: 15, description: "Pre-Nursery to Class 12" },
];
...
export default function SchoolSetup() {
  ...
  const handleLogoUpload = async (e) => {
    ...
    try {
      ...
      if (data.url) {
        setForm(f => ({ ...f, schoolLogoUrl: data.url }));
        setLogoPreview(data.url); // Use the URL returned by the server for preview
      }
    } catch (err) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        schoolCode: "SYSTEM_GEN", // Backend will generate
        classLevel: parseInt(form.classLevel),
        classLevelStart: 0, // Automated setup starts from level 0 (Pre-Nursery)
          ...
          <AnimatePresence mode="wait">
            ...
            {step === 1 && (
              <motion.div key="step1" variants={fadeInVariants} initial="hidden" animate="visible" exit="exit" className="w-full max-w-sm mx-auto">
                ...
                <div className="space-y-6">
                  {/* Logo Upload Section */}
                  <div className="flex flex-col items-center justify-center p-6 border-2 border-dashed border-white/10 rounded-2xl bg-white/[0.02] hover:bg-white/[0.04] transition-all group relative overflow-hidden">
                    <input
                      type="file"
                      id="logo-upload"
                      className="hidden"
                    ...
                    {logoPreview ? (
                      <div className="relative w-24 h-24 rounded-2xl overflow-hidden border-2 border-primary/30 shadow-2xl">
                        <img src={logoPreview} alt="School Logo" className="w-full h-full object-cover" />
                        <button
                          onClick={() => { setLogoPreview(null); setForm(f => ({ ...f, schoolLogoUrl: "" })); }}
                          className="absolute top-1 right-1 p-1 bg-rose-500 rounded-full text-white hover:scale-110 transition-transform z-10"
                        >
                          <X size={12} />
                        </button>
                        {isUploadingLogo && (
                      ...
                      <label htmlFor="logo-upload" className="cursor-pointer flex flex-col items-center gap-2">
                        <div className="w-16 h-16 rounded-2xl bg-primary/10 flex items-center justify-center text-primary group-hover:scale-110 transition-transform">
                          {isUploadingLogo ? <Loader className="w-8 h-8 animate-spin" /> : <Upload size={32} />}
                        </div>
                        <div className="text-center">
                          <p className="text-sm font-bold text-white uppercase tracking-wider">Institution Logo</p>
                          <p className="text-[10px] text-slate-500 font-medium uppercase tracking-widest mt-1">PNG, JPG or WebP (Max 5MB)</p>
                        </div>
                      </label>
                    )}
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-1.5">
                      <label className="text-[10px] text-slate-500 font-bold uppercase tracking-[0.2em] ml-1">Official School Name</label>
                      <div className="relative group">
                        ...
                        <input
                          ...
                          className="w-full bg-slate-900/50 border border-white/10 rounded-xl py-3.5 pl-12 pr-4 text-white placeholder:text-slate-600 focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary/50 transition-all font-medium"
                    ...
                    {/* Established Year */}
                    <div className="space-y-1.5">
                      <label className="text-[10px] text-slate-500 font-bold uppercase tracking-[0.2em] ml-1">Establishment Year</label>
                      <div className="relative group">
                        <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-500 group-focus-within:text-primary transition-colors">
                          <Calendar size={18} />
                        </div>
                        <input
                          type="number"
                          name="sinceEstablished"
                          placeholder="YYYY"
                          className="w-full bg-slate-900/50 border border-white/10 rounded-xl py-3.5 pl-12 pr-4 text-white placeholder:text-slate-600 focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary/50 transition-all font-medium"
                          value={form.sinceEstablished}
                          onChange={handleChange}
                        />
                      </div>
                    </div>
                    ...
                    <div className="space-y-1.5">
                      <div className="flex items-center justify-between ml-1 mb-1">
                        <label className="text-[10px] text-slate-500 font-bold uppercase tracking-[0.2em]">Director(s)</label>
                        {form.directors.length < 5 && (
                          <button type="button" onClick={addDirector} className="flex items-center gap-1 text-[10px] text-primary hover:text-primary/80 font-bold uppercase tracking-widest transition-colors">
                        ...
                        {form.directors.map((director, idx) => (
                          <div key={idx} className="relative flex items-center gap-2 group">
                            ...
                            <input
                              ...
                              className="w-full bg-slate-900/50 border border-white/10 rounded-xl py-3 pl-10 pr-4 text-sm text-white placeholder:text-slate-600 focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all font-medium"
                              placeholder={`Director ${idx + 1}`}
                            />
                            {form.directors.length > 1 && (
                              <button type="button" onClick={() => removeDirector(idx)} className="p-2.5 rounded-xl text-slate-500 hover:text-rose-400 hover:bg-rose-500/10 transition-all">
                                <X size={16} />
                              </button>
                            )}
                          </div>
                  ...
                  <div className="pt-4">
                    <button onClick={handleNext} className="btn-primary w-full py-4 text-sm font-bold uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg shadow-primary/20">
                      Continue Strategy <ArrowRight size={18} />
                    </button>
                  </div>
                </div>
              </motion.div>
