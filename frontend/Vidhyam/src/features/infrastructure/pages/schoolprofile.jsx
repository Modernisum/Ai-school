    ...
    const handleLogoUpload = async (e) => {
        ...
        try {
            ...
            if (data.url) {
                ...
                showToast('success', 'Logo uploaded! Click commit to save.');
            }
        } catch (err) {
        ...
        <div className="min-h-full pb-12">
            <div className="flex items-center justify-between px-6 py-3">
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-slate-900 border border-white/10 flex items-center justify-center shadow-md overflow-hidden group">
                        {school.schoolLogoUrl ? (
                            <img src={school.schoolLogoUrl} alt="Logo" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                        ) : (
                    ...
                    <div>
                        <h1 className="text-base font-bold text-white tracking-tight uppercase leading-tight">Institutional Profile</h1>
                        <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest leading-none mt-0.5">Registry ID: {schoolId}</p>
                    </div>
                </div>
                <div className="flex items-center gap-2">
                    <button onClick={() => refetch()} className={`p-1.5 rounded-lg bg-white/5 border border-white/10 text-slate-400 hover:text-white transition-all ${isFetching ? 'animate-spin opacity-50' : ''}`}>
                        <RefreshCw size={14} />
                    </button>
                    <button onClick={handleSignOut} className="flex items-center gap-2 px-4 py-1.5 rounded-lg bg-accent/10 hover:bg-accent/20 border border-accent/20 text-accent text-[10px] font-black uppercase tracking-widest transition-all shadow-lg shadow-accent/5">
                        <LogOut size={13} /> De-Authenticate
                    </button>
                </div>
            </div>
                        ...
                        {editSection === 'identity' ? (
                            <div className="space-y-4 pt-2">
                                ...
                                <div className="space-y-1.5">
                                    <label className="text-[10px] text-slate-500 font-black uppercase tracking-widest ml-1">INSTITUTION LOGO</label>
                                    <div className="flex items-center gap-4 p-3 bg-white/[0.02] border border-white/10 rounded-xl">
                                        <div className="w-12 h-12 rounded-lg bg-slate-900 overflow-hidden border border-white/10">
                                            {draft.schoolLogoUrl ? (
                                                <img src={draft.schoolLogoUrl} alt="Preview" className="w-full h-full object-cover" />
                                            ) : (
                                        ...
                                        <div className="flex-1">
                                            <input type="file" id="profile-logo-upload" className="hidden" accept="image/*" onChange={handleLogoUpload} />
                                            <label htmlFor="profile-logo-upload" className="cursor-pointer inline-flex items-center gap-2 px-3 py-1.5 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg text-[10px] font-black uppercase tracking-widest text-slate-300 transition-all">
                                                {isUploadingLogo ? <Loader size={12} className="animate-spin" /> : <Upload size={12} />}
                                                Change Logo
                                            </label>
                                        </div>
                                    </div>
