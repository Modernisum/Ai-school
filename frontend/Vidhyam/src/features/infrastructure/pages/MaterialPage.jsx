...
import {
  ...
  ArrowUpRight, ArrowDownRight, ListOrdered, Edit3, Shield, Activity, Layers,
} from "lucide-react";
...
function MaterialPage({ schoolId, pollingInterval }) {
  ...
  const onTransactionSubmit = async (data) => {
    ...
    try {
      const body = { quantity: parseInt(data.quantity), unitPrice: parseFloat(data.unitPrice), notes: transactionType === "buy" ? "Procurement" : "Distribution" };
    ...
    <div className="max-w-full p-1 space-y-2">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-1">
        {isFetching && page === 1 ? (
          ...
          materials.map((m, i) => (
            <MaterialCard
              key={m.materialName || i}
              material={m}
              schoolId={schoolId}
              onEdit={(mat) => { setEditingMaterial(mat); resetMaterial(mat); }}
              onDelete={handleDeleteMaterial}
              onTransactionIn={() => { setTransactionType("buy"); setTransactionMaterial(m); setTransactionValue("quantity", 1); setTransactionValue("unitPrice", m.unitPrice); }}
              onTransactionOut={() => { setTransactionType("sell"); setTransactionMaterial(m); setTransactionValue("quantity", 1); setTransactionValue("unitPrice", m.unitPrice); }}
            />
          ))
        )}
      </div>
      ...
      <div className="flex items-center justify-between pt-2">
        {metadata.totalCount > limit && (
          <div className="flex gap-1">
            <StandardButton icon={ChevronLeft} size="xs" variant="ghost" onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} />
            <span className="px-2 text-micro font-black text-slate-700 leading-8">PAGE {page} OF {metadata.totalPages}</span>
            <StandardButton icon={ChevronRight} size="xs" variant="ghost" onClick={() => setPage(p => Math.min(metadata.totalPages, p + 1))} disabled={page >= metadata.totalPages} />
          </div>
        )}
      </div>
      ...
      <AnimatePresence>
        {(isAddingMaterial || editingMaterial) && (
          <div className="fixed inset-0 z-[120] flex items-center justify-end p-8 pointer-events-none">
            ...
            <motion.div initial={{ x: 100, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 100, opacity: 0 }}
              ...
              <FormWidget
                title={editingMaterial ? "MODIFY_ASSET" : "PROVISION_ASSET"}
                sections={MATERIAL_SCHEMA}
                control={materialControl}
                onSubmit={handleMaterialSubmit(editingMaterial
                  ? (data) => handleUpdateMaterial(editingMaterial.materialName || editingMaterial.id, data)
                  : handleCreateMaterial)}
                onCancel={() => { setIsAddingMaterial(false); setEditingMaterial(null); }}
                submitLabel={editingMaterial ? "COMMIT_CHANGES" : "INITIALIZE_RESOURCE"}
                dense
              />
            </motion.div>
          </div>
        ...
        {transactionMaterial && transactionType && (
          <div className="fixed inset-0 z-40 flex items-center justify-center p-4 backdrop-blur-sm">
            <GlassCard dense glowColor={transactionType === "buy" ? "success" : "warning"} className="w-full max-w-md p-4">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-sm font-black uppercase tracking-widest">{transactionType === "buy" ? "PROCURE" : "DISTRIBUTE"} — {transactionMaterial.materialName}</h3>
                <button onClick={() => { setTransactionType(""); setTransactionMaterial(null); }} className="text-slate-500 hover:text-white">
                  <X size={16} />
                </button>
              </div>
              <FormWidget
                ...
                onCancel={() => { setTransactionType(""); setTransactionMaterial(null); }}
                submitLabel={transactionType === "buy" ? "EXECUTE_PROCUREMENT" : "EXECUTE_DISTRIBUTION"}
                dense
                noTabs
              />
            </GlassCard>
...
// ── Individual Material Card Component ──
function MaterialCard({ material: m, schoolId, onEdit, onDelete, onTransactionIn, onTransactionOut }) {
  return (
    <>
    <GlassCard hover delay={0} className="group flex flex-col h-full bg-white/[0.02]" glowColor="primary" dense>
      <div className="p-1 flex flex-col h-full">
        ...
        <div className="mt-auto pt-1 border-t border-white/5 space-y-1">
          ...
          {/* Material ↔ Space bridge: show spaces using this material */}
          <div className="flex items-center justify-between">
            <p className="text-[7px] font-black text-slate-800 uppercase tracking-widest leading-none">SPACES</p>
            <span className="text-[7px] font-black text-primary/60 leading-none flex items-center gap-1">
              <Layers size={6} />
              {m.space_count || m.assignedSpaces || "0"}
            </span>
          </div>
          <div className="flex gap-1">
            <StandardButton label="IN" size="xs" className="flex-1 bg-emerald-500/10 text-emerald-500 border-emerald-500/10" onClick={onTransactionIn} />
            <StandardButton label="OUT" size="xs" className="flex-1 bg-amber-500/10 text-amber-500 border-amber-500/10" onClick={onTransactionOut} />
          </div>
        </div>
      </div>
    </GlassCard>
