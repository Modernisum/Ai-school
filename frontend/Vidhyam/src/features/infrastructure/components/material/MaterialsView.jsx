...
import { Search, Plus, DollarSign, AlertTriangle, Package, RefreshCw } from 'lucide-react';
...
function MaterialsView({ materials, schoolId, onAdd, onEdit, onDelete, onBuy, onSell, onViewHistory, search, setSearch, filter, setFilter }) {
  ...
  const filtered = materials.filter(m => {
    ...
    const stockPercentage = m.quantity > 0 ? (m.extraUnit / m.quantity) * 100 : 0;
    if (filter === 'Low Stock') return matchesSearch && stockPercentage < 20 && m.extraUnit > 0;
    if (filter === 'Out of Stock') return matchesSearch && m.extraUnit <= 0;
    if (filter === 'Shortage') return matchesSearch && m.needUnit > 0;
  ...
  return (
    <div className="p-8 space-y-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-8">
        {filtered.map(m => <MaterialCard key={m.id} material={m} onEdit={() => onEdit(m)} onDelete={() => onDelete(m.id)} onBuy={() => onBuy(m)} onSell={() => onSell(m)} onViewHistory={() => onViewHistory(m)} />)}
        {filtered.length === 0 && <div className="col-span-full py-40 text-center bg-white/[0.01] border border-white/5 border-dashed rounded-[3rem] shadow-inner"><Package size={64} className="text-slate-800 mx-auto mb-6 opacity-40" /><p className="text-slate-500 font-black text-sm uppercase tracking-[0.3em] italic">No Assets Found in Inventory Register</p></div>}
      </div>
    </div>
  );
}
