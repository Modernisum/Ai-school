  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 backdrop-blur-3xl bg-slate-950/60">
      <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 20, opacity: 0 }} className="w-full max-w-lg">
        <GlassCard title={step === 'select' ? 'SELECT MATERIAL TO TRANSFER' : 'TRANSFER_MATERIAL'} onClose={onClose} className="p-4" glowColor="primary" dense>
          {step === 'select' ? (
            <div className="space-y-2 mt-2 max-h-64 overflow-y-auto">
              <p className="text-[8px] font-black text-slate-700 uppercase tracking-widest mb-1">MATERIALS IN {fromSpace?.spaceName || fromSpace?.name}</p>
              {sourceMats.length === 0 ? (
                ...
                sourceMats.map((mat, i) => (
                  <button key={i} onClick={() => handleSelectMaterial(mat)}
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[9px] font-black text-white uppercase">{mat.materialName}</span>
                    </div>
                    <span className="text-[8px] font-black text-slate-600">Qty: {mat.quantity}</span>
                  </button>
                ))
              )}
            </div>
            ...
            <div className="space-y-3 mt-2">
              <div className="flex items-center gap-2 p-2 rounded-lg bg-white/5">
                <div className="flex-1">
                  <p className="text-[7px] font-black text-slate-600 uppercase tracking-widest">FROM</p>
                  <p className="text-[9px] font-black text-white uppercase tracking-tight">{fromSpace?.spaceName || fromSpace?.name}</p>
                </div>
                <ArrowRight size={14} className="text-primary" />
                <div className="flex-1">
                  <p className="text-[7px] font-black text-slate-600 uppercase tracking-widest">MATERIAL</p>
                  <p className="text-[9px] font-black text-white uppercase tracking-tight">{selectedMaterial?.materialName}</p>
                </div>
              </div>
              <div>
                <p className="text-[8px] font-black text-slate-700 uppercase tracking-widest mb-1">TARGET_SPACE</p>
                <select
              ...
              <div>
                <p className="text-[8px] font-black text-slate-700 uppercase tracking-widest mb-1">QUANTITY (max: {maxQuantity})</p>
                <input
                  type="number"
                  min={1}
                  max={maxQuantity}
