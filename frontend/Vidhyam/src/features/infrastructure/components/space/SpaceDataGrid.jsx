    ...
    <div className="space-y-1">
      <div className="flex flex-col md:flex-row gap-2 items-center justify-between bg-white/5 p-1.5 rounded-xl border border-white/10">
        ...
        <div className="flex items-center gap-1">
          <select
            className="bg-slate-900 border border-white/10 rounded-lg px-2 py-1.5 text-[8px] font-black text-white uppercase tracking-tight focus:outline-none focus:border-primary/50"
            value={categoryFilter}
            onChange={e => setCategoryFilter(e.target.value)}
          >
            {categories.map(c => (
              <option key={c} value={c}>{c === 'all' ? 'ALL_CATEGORIES' : c.toUpperCase()}</option>
            ))}
          </select>
          <select
            className="bg-slate-900 border border-white/10 rounded-lg px-2 py-1.5 text-[8px] font-black text-white uppercase tracking-tight focus:outline-none focus:border-primary/50"
      ...
      <div className="space-y-0.5">
        {paginated.length === 0 ? (
          <div className="py-8 text-center glass-card border-dashed flex flex-col items-center justify-center opacity-30">
            <Box size={20} className="mb-2" />
            <p className="text-[9px] font-black uppercase tracking-[0.4em]">NO_SPACES_FOUND</p>
          </div>
        ) : (
          paginated.map((space, i) => {
            ...
            const statusColor = status === 'full' ? 'text-green-400' : status === 'deficient' ? 'text-amber-400' : 'text-slate-600';
            const spaceMats = materialsBySpace?.[name] || [];
            const totalValue = spaceMats.reduce((sum, m) => sum + (m.unitPrice || 0) * (m.quantity || 0), 0);
            return (
              <GlassCard key={space.spaceId || space.id || i} hover delay={i * 0.005} dense className="bg-white/[0.02]">
                <div className="flex items-center justify-between p-1.5">
                  <div className="flex items-center gap-2 flex-1 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="text-[9px] font-black text-white uppercase tracking-tighter truncate">{name}</p>
                      <div className="flex items-center gap-1 mt-0.5">
                        <span className="text-[7px] font-black text-primary/40 uppercase tracking-widest">{space.spaceCategory || 'GENERAL'}</span>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <div className="text-right hidden sm:block">
                      <p className="text-[7px] font-black text-slate-700 uppercase tracking-widest">{respCount} RSP</p>
                    </div>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-1 pt-1">
            ...
            return (
              <button
                ...
                className={`px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-wider transition-all ${
                  pageNum === page ? 'bg-primary text-white' : 'bg-white/5 text-slate-600 hover:text-white'
                }`}
              >
                {pageNum}
              </button>
          ...
          <span className="text-[7px] font-black text-slate-700 ml-1">/{totalPages}</span>
        </div>
      )}
    </div>
  );
