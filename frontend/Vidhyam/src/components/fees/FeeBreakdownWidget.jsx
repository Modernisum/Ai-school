        ...
        <div className="flex gap-1">
          <button
            ...
            className={`px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider rounded transition-colors ${
              viewMode === 'monthly' ? 'bg-primary/20 text-primary' : 'text-slate-500 hover:text-slate-300'
            }`}
          >
            Monthly
          </button>
          <button
            ...
            className={`px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider rounded transition-colors ${
              viewMode === 'yearly' ? 'bg-primary/20 text-primary' : 'text-slate-500 hover:text-slate-300'
            }`}
          >
            Yearly
          </button>
      ...
      <div className="space-y-1">
            ...
            {mandatoryFees.map((f) => {
              ...
              return (
                <div key={f.responsibilityId || f.id} className="flex items-center justify-between py-1 text-xs">
                  ...
                  <span className="text-amber-300">{formatCurrency(fee)}/{viewMode === 'yearly' ? 'yr' : 'mo'}</span>
                </div>
              );
            })}
            <div className="flex justify-between py-1 text-xs font-bold border-t border-amber-500/10 mt-0.5">
              <span className="text-amber-300">Subtotal (Mandatory)</span>
              <span className="text-amber-300">{formatCurrency(totalMandatory * multiplier)}</span>
            </div>
          </div>
        )}
        ...
        {optionalFees?.length > 0 && (
          <div className="border-t border-white/5 pt-2 mt-1">
            ...
            {totalOptional > 0 && (
              <div className="flex justify-between py-1 text-xs font-bold border-t border-primary/10 mt-0.5">
        ...
        <div className="flex justify-between items-center border-t border-white/10 pt-2 mt-1">
          ...
          <span className="text-sm font-black text-emerald-400">{formatCurrency(grandTotal)}</span>
        </div>
      </div>
    </div>
  );
