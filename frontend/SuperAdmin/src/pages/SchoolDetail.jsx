...
import { ArrowLeft, Save, Ban, CheckCircle, Key, Clock, Loader, Edit3, X, CreditCard, Users, TrendingUp, Upload } from 'lucide-react'
import { ToastCtx } from '../App.jsx'
import { getSchool, updateSchool, setStatus, setSessionDuration, applyPromo, uploadFile, deleteFileByUrl } from '../api.js'
...
export default function SchoolDetail() {
    ...
    const load = async () => {
        ...
        if (r.data) {
            ...
            setEdits({
                ...
                schoolLogoUrl: r.data.schoolLogoUrl || d.schoolLogoUrl || '',
            })
        }
        setLoading(false)
    }
    ...
    const saveChanges = async () => {
        setSaving(true)
        setLogoLoading(true); // Start logo loading indicator for the upload part
        try {
            let finalLogoUrl = edits.schoolLogoUrl;
            ...
            const r = await updateSchool(schoolId, {
                ...edits,
    ...
    const cancelEdit = () => {
        ...
        setEdits({
            ...
            affiliatedBoard: d.affiliatedBoard || '',
            schoolType: d.schoolType || '',
            schoolLogoUrl: school?.schoolLogoUrl || d.schoolLogoUrl || '',
        })
        setEditing(false)
    }
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                    {school.schoolLogoUrl && (
                        <img 
                            ...
                            alt="Logo" 
                            style={{ width: 48, height: 48, borderRadius: 10, objectFit: 'cover', border: '1px solid var(--bg3)' }} 
                        />
                    )}
                    <div>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                            <div className="input-group" style={{ gridColumn: '1 / -1' }}>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginTop: 8 }}>
                                    {(localLogoPreview || edits.schoolLogoUrl) ? (
                                        <div style={{ position: 'relative', width: 64, height: 64, borderRadius: 8, overflow: 'hidden', border: '1px solid var(--bg3)' }}>
                                            <img src={localLogoPreview || edits.schoolLogoUrl} alt="Preview" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                                            <button 
                                                type="button" 
                                                style={{ position: 'absolute', top: 2, right: 2, background: 'rgba(0,0,0,0.5)', border: 'none', color: 'white', borderRadius: '50%', padding: 2, cursor: 'pointer' }}
                                                onClick={() => {
                                                    // Only delete from server if it was already a server URL
                                                    if (edits.schoolLogoUrl && !localLogoPreview) {
                                                        deleteFileByUrl(edits.schoolLogoUrl);
                                                    }
                                                    set('schoolLogoUrl', '');
                                                    setLocalLogoPreview('');
                                                    setPendingLogoFile(null);
