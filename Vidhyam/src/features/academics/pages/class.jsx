...
import { useGetClassesQuery, useAddClassMutation, useDeleteClassMutation } from '../api/academicApi';

export default function ClassManagement() {
  const schoolId = getSchoolIdFromStorage() || "622079";
  const { data: classes = [], isLoading: loading, refetch } = useGetClassesQuery(schoolId);
  ...
  const loadClasses = () => refetch();

  const addClass = async () => {
    if (!newClassName.trim()) return;
    try {
      await addClassMutation({ schoolId, className: newClassName.trim() }).unwrap();
  ...
  const deleteClass = async (id) => {
    try {
      await deleteClassMutation({ schoolId, classId: id }).unwrap();
