...
impl DocumentBoxRepository for PostgresDocumentBoxRepository {
    async fn add_document(&self, school_id: &str, data: Value) -> Result<Value, AppError> {
        ...
        let res = sqlx::query("INSERT INTO document_box (school_id, user_id, doc_type, file_url) VALUES ($1, $2, $3, $4) RETURNING id")
            .bind(school_id)
    ...
    async fn get_documents(
        ...
        let rows = if let Some(sid) = student_id {
            sqlx::query("SELECT id, doc_type as title, file_url FROM document_box WHERE school_id = $1 AND user_id = $2")
                .bind(school_id).bind(sid).fetch_all(&mut *conn).await?
        } else {
            sqlx::query("SELECT id, doc_type as title, file_url FROM document_box WHERE school_id = $1")
                .bind(school_id)
                .fetch_all(&mut *conn)
                .await?
        };
    ...
    async fn get_document(
        ...
        let mut conn = self.client.acquire_tenant_connection(school_id).await?;
        let row = sqlx::query("SELECT id, doc_type as title, file_url FROM document_box WHERE school_id = $1 AND id = $2")
            .bind(school_id)
            .bind(document_id)
