...
pub struct SchoolManagementService {
    pub db: std::sync::Arc<crate::db::DbClient>,
}

impl SchoolManagementService {
    ...
    pub async fn delete_school(&self, school_id: &str) -> Result<(), Box<dyn Error + Send + Sync>> {
        ...
        for table in &tables {
            ...
            // Create a savepoint before attempting deletion
            ...
            if let Err(e) = sqlx::query(&format!("DELETE FROM {} WHERE school_id = $1", table))
                ...
                .await {
                    ...
                    // Rollback to savepoint so main transaction is not aborted
        ...
        // Finally delete school record itself
