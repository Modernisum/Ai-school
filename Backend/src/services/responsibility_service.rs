...
impl ResponsibilityService for PostgresResponsibilityService {
        ...
        if let Some(employees) = data["employees"].as_array() {
            for emp in employees {
                if let Some(emp_id) = emp["employeeId"].as_str() {
                    ...
                    let assignments = self.repos.responsibility.get_employee_responsibilities(school_id, emp_id).await?;
                    let responsibility_id_from_name = name.to_uppercase()
                        .chars()
    ...
    async fn generate_utilization_report_pdf(
        ...
        let report = self.generate_utilization_report(school_id, start_date, end_date).await?;
        let title = format!("Responsibility Utilization Report - {} to {}", start_date, end_date);
        
        let pdf_data = PdfGenerator::generate_report(&title, &report)
    ...
    async fn generate_workload_report_pdf(
        ...
        let report = self.generate_workload_report(school_id, start_date, end_date).await?;
        let title = format!("Employee Workload Report - {} to {}", start_date, end_date);
    ...
    async fn generate_space_distribution_report_pdf(
        ...
        let report = self.generate_space_distribution_report(school_id, start_date, end_date).await?;
    ...
    async fn generate_revenue_report_pdf(
        ...
        let report = self.generate_revenue_report(school_id, start_date, end_date).await?;
