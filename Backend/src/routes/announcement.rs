...
pub async fn create_announcement(
    ...
    if role == "TEACHER" {
        ...
        if let (Some(cid), Some(sid)) = (class_id, subject_id) {
            // Get class details
            let class = state.repos.academic.get_class(&school_id, cid).await?
                .ok_or_else(|| AppError::Validation(format!("Class '{}' does not exist", cid)))?;
            let class_name = class["className"].as_str().unwrap_or("");

            // Get subject details
            let subject = state.repos.academic.get_subject(&school_id, sid).await?
                .ok_or_else(|| AppError::Validation(format!("Subject '{}' does not exist", sid)))?;
            let subject_name = subject["name"].as_str().unwrap_or("");

            // Validate responsibility mapping: Name must match "{subject_name} - {class_name}"
            ...
            let has_access = responsibilities.iter().any(|r| {
                if let Some(r_name) = r.get("name").and_then(|v| v.as_str()) {
            ...
            if !has_access {
                return Err(AppError::Forbidden(format!(
                    "Teacher is not mapped to this class/subject responsibility: '{}'",
                    expected_name
                )));
            }
        } else {
    ...
    if let Some(title) = payload.get("title").and_then(|v| v.as_str()) {
        if let Some(content) = payload.get("content").and_then(|v| v.as_str()) {
            ...
            if let Some(cid) = class_id {
                if let Ok(Some(class)) = state.repos.academic.get_class(&school_id, cid).await {
                    if let Some(class_name) = class["className"].as_str() {
