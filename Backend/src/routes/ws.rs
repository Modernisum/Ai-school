...
async fn handle_socket(mut socket: WebSocket, state: AppState) {
    ...
    let _ = socket.send(Message::Text("Authenticated successfully".into())).await;
        ...
        if let Message::Text(text) = msg {
            if let Ok(payload) = serde_json::from_str::<WsAuthPayload>(&text) {
                ...
                if let Ok(Some(token_data)) = state.repos.auth.get_token(&payload.token).await {
                    let u_id = token_data["tokenId"].as_str().unwrap_or("unknown").to_string(); // In a real app, map token to user_id properly
