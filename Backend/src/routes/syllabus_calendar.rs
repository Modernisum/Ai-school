...
pub async fn plot_annual(
    ...
    Path((school_id, class_id, subject_id)): Path<(String, String, String)>,
    ...
    match planner
        .annual_syllabus_plot(&school_id, &class_id, &subject_id, academic_year)
        .await
    {
...
pub async fn micro_plan_periods(
    ...
    match planner
        .micro_plan_period_level(&school_id, &class_id, &subject_id, from_date, to_date)
        .await
    {
        Ok(data) => Json(json!(data)).into_response(),
        Err(e) => (
...
pub async fn get_syllabus(
    ...
    let rows = sqlx::query(
        "SELECT sc.*, c.name as chapter_name FROM syllabus_calendar sc \
         ...
         WHERE sc.school_id = $1 AND sc.subject_id = $2 ORDER BY sc.quarter, sc.planned_start_date"
    )
