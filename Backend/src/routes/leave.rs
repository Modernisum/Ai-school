...
pub async fn download_leave_pdf(
    ...
    let leaves = match state.services.leave.get_leaves(&school_id).await {
        Ok(l) => l,
        Err(_) => return (axum::http::StatusCode::INTERNAL_SERVER_ERROR, Json(json!({"success": false, "message": "Failed"}))).into_response(),
    };

    let leave = match leaves.into_iter().find(|l| l["id"].as_str() == Some(&leave_id) || l["leaveId"].as_str() == Some(&leave_id)) {
        Some(l) => l,
        None => return (axum::http::StatusCode::NOT_FOUND, Json(json!({"success": false, "message": "Leave not found"}))).into_response(),
    };

    let applicant_name = leave["applicantName"].as_str().unwrap_or(leave["studentName"].as_str().unwrap_or("Employee/Student"));
    ...
    current_layer.use_text("OFFICIAL LEAVE LETTER", 24.0, Mm(65.0), Mm(260.0), &font_bold);
    current_layer.use_text(format!("Applicant: {}", applicant_name), 14.0, Mm(20.0), Mm(230.0), &font);
    current_layer.use_text(format!("Role: {}", role), 14.0, Mm(20.0), Mm(220.0), &font);
    current_layer.use_text(format!("Start Date: {}", start_date), 14.0, Mm(20.0), Mm(210.0), &font);
    current_layer.use_text(format!("End Date: {}", end_date), 14.0, Mm(20.0), Mm(200.0), &font);
    
    let status_color = if status == "APPROVED" { Color::Rgb(Rgb::new(0.0, 0.6, 0.0, None)) } else if status == "REJECTED" { Color::Rgb(Rgb::new(0.8, 0.0, 0.0, None)) } else { Color::Rgb(Rgb::new(0.5, 0.5, 0.5, None)) };
    current_layer.set_fill_color(status_color);
    current_layer.use_text(format!("STATUS: {}", status), 16.0, Mm(20.0), Mm(185.0), &font_bold);
    ...
    if !current_line.is_empty() { current_layer.use_text(&current_line, 12.0, Mm(20.0), Mm(y_pos), &font); }
    
    current_layer.use_text("Authorized Signature: _______________________", 12.0, Mm(20.0), Mm(y_pos - 40.0), &font);
    ...
    let response = Response::builder()
        ...
        .header(header::CONTENT_DISPOSITION, format!("attachment; filename=\"Leave_Letter.pdf\""))
        .body(Body::from(buf))
        .unwrap();
