...
pub async fn suggest_substitute(
    State(state): State<AppState>,
    Path((school_id, class_id, subject_id, day, period)): Path<(String, String, String, usize, usize)>,
) -> impl IntoResponse {
    let engine = TimetableEngine::new(state.db.pool.clone());
    match engine.find_best_substitute(&school_id, &class_id, &subject_id, day, period).await {
        Ok(candidates) => {
