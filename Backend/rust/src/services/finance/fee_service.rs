...
impl FeeService for PostgresFeeService {
    async fn create_school_fee(
        ...
        log_audit(&self.repos.audit, school_id, admin_id, "SCHOOL_FEE", fee_id, "CREATE", data).await;
        Ok(res)
    }
    ...
    async fn add_fee_to_student(
        ...
        log_audit(&self.repos.audit, school_id, admin_id, "STUDENT_FEE_ADDITION", student_id, "ADD_FEE", json!({"amount": amount, "feeId": fee_id})).await;
        Ok(fee_record)
    }
    ...
    async fn apply_discount(
        ...
        log_audit(&self.repos.audit, school_id, admin_id, "STUDENT_FEE_DISCOUNT", student_id, "APPLY_DISCOUNT", json!({"discount": discount})).await;
        Ok(fee_record)
    }
    ...
    async fn pay_fee(
        ...
        log_audit(&self.repos.audit, school_id, admin_id, "STUDENT_FEE_PAYMENT", student_id, "PAY", payload).await;
        Ok(fee_record)
    }
    ...
    async fn create_custom_fee(
        ...
        log_audit(&self.repos.audit, school_id, admin_id, "CUSTOM_FEE", &id, "CREATE", data).await;
        Ok(res)
    }
    ...
    async fn remove_custom_fee(&self, school_id: &str, fee_id: &str, admin_id: &str) -> AppResult<()> {
        ...
        log_audit(&self.repos.audit, school_id, admin_id, "CUSTOM_FEE", fee_id, "DELETE", json!({})).await;
        Ok(())
    }
    ...
    async fn apply_custom_fee(&self, school_id: &str, fee_id: &str, admin_id: &str) -> AppResult<Value> {
        ...
        log_audit(&self.repos.audit, school_id, admin_id, "CUSTOM_FEE_APPLY", fee_id, "APPLY", json!({})).await;
        Ok(res)
    }
...
impl CouponService for PostgresFeeService {
    async fn create_coupon(&self, school_id: &str, admin_id: &str, data: Value) -> AppResult<Value> {
        ...
        let id = res["id"].as_i64().map(|id| id.to_string()).unwrap_or_else(|| "0".to_string());
        log_audit(&self.repos.audit, school_id, admin_id, "COUPON", &id, "CREATE", data).await;
        Ok(res)
    }
    ...
    async fn remove_coupon(&self, school_id: &str, coupon_id: &str, admin_id: &str) -> AppResult<()> {
        ...
        log_audit(&self.repos.audit, school_id, admin_id, "COUPON", coupon_id, "DELETE", json!({})).await;
        Ok(())
    }
    ...
    async fn toggle_block_coupon(&self, school_id: &str, coupon_id: &str, admin_id: &str, blocked: bool) -> AppResult<()> {
        ...
        log_audit(&self.repos.audit, school_id, admin_id, "COUPON_BLOCK", coupon_id, action, json!({})).await;
        Ok(())
    }
    ...
    async fn use_coupon(&self, school_id: &str, coupon_id: &str, student_id: &str, admin_id: &str, discount: f64) -> AppResult<Value> {
        ...
        log_audit(&self.repos.audit, school_id, admin_id, "COUPON_USE", coupon_id, "USE", json!({"studentId": student_id, "discount": discount})).await;
        Ok(res)
    }
}
