...
impl AuthService for PostgresAuthService {
    async fn login(&self, data: Value) -> AppResult<Value> {
        ...
        if let Some(a) = auth {
            ...
            if crate::logic::password_helper::verify_password(password, hashed).map_err(|e| AppError::Internal(format!("Auth error: {}", e)))? {
    ...
    async fn set_security(
        ...
        let hashed_answer = hash_password(&answer.trim().to_lowercase())?;
        let data = json!({
    ...
    async fn forgot_password(
        ...
        if let Some(a) = auth {
            ...
            if verify_password(&answer.trim().to_lowercase(), hashed_answer)? {
                let temp_pass = format!("{:08}", rand::random::<u32>() % 100000000);
                let hashed_temp = hash_password(&temp_pass)?;
                self.repos
                    .auth
                    .update_auth(
                        school_id,
    ...
    async fn change_password(
        ...
        if let Some(a) = auth {
            let hashed = a["password"].as_str().unwrap_or("");
            if verify_password(old_pass, hashed)? {
                let hashed_new = hash_password(new_pass)?;
                self.repos
                    .auth
    ...
    async fn change_password_self(&self, school_id: &str, new_password: &str) -> AppResult<()> {
        ...
        self.repos.auth.change_password_tx(school_id, &hashed).await?;
        Ok(())
    }
