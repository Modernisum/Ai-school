...
impl AiRepository for PostgresAiRepository {
    ...
    async fn search_similar_documents(
        ...
        let rows = sqlx::query(
            "SELECT chunk_text as content, chunk_embedding::real[] as embedding FROM document_embeddings \
             WHERE school_id = $1 \
             ORDER BY chunk_embedding <=> $2::real[]::vector \
             LIMIT $3"
        )
        .bind(school_id)
    ...
    async fn store_document_embedding(
        ...
        let mut conn = self.client.acquire_tenant_connection(school_id).await?;
        let document_id = metadata.get("document_id")
            .and_then(|v| v.as_str())
            .unwrap_or("default");

        let row = sqlx::query(
            "INSERT INTO document_embeddings (school_id, document_id, chunk_text, chunk_embedding, created_at) \
             VALUES ($1, $2, $3, $4::real[]::vector, NOW()) \
             RETURNING id"
        )
        .bind(school_id)
        .bind(document_id)
    ...
    async fn get_cached_sql(&self, school_id: &str, query_embedding: &[f32], threshold: f64) -> Result<Option<(String, String)>, AppError> {
        let mut conn = self.client.acquire_tenant_connection(school_id).await?;
        let row = sqlx::query(
            "SELECT generated_sql, question_text FROM ai_query_cache \
             WHERE (school_id = $1 OR school_id = 'GLOBAL') \
               AND 1 - (question_embedding <=> $2::real[]::vector) > $3 \
             ORDER BY question_embedding <=> $2::real[]::vector ASC \
             LIMIT 1"
        )
        .bind(school_id)
        .bind(query_embedding)
        ...
        if let Some(r) = row {
            let sql: String = r.get("generated_sql");
            let q_text: String = r.get("question_text");
            Ok(Some((sql, q_text)))
        } else {
            Ok(None)
        }
    ...
    async fn save_cached_sql(&self, school_id: &str, question_text: &str, generated_sql: &str, query_embedding: &[f32], save_global: bool) -> Result<(), AppError> {
        ...
        // 1. Save specific cache
        sqlx::query(
            "INSERT INTO ai_query_cache \
             (school_id, question_text, generated_sql, question_embedding) \
             VALUES ($1, $2, $3, $4::real[]::vector) \
             ON CONFLICT (school_id, question_text) DO UPDATE SET search_count = ai_query_cache.search_count + 1, last_used_at = NOW()"
        )
        .bind(school_id)
        .bind(question_text)
        .bind(generated_sql)
        ...
        if save_global {
            let global_sql = generated_sql.replace(school_id, "{sid}");
            sqlx::query(
                "INSERT INTO ai_query_cache \
                 (school_id, question_text, generated_sql, question_embedding) \
                 VALUES ('GLOBAL', $1, $2, $3::real[]::vector) \
                 ON CONFLICT (school_id, question_text) DO UPDATE SET search_count = ai_query_cache.search_count + 1, last_used_at = NOW()"
            )
            .bind(question_text)
            .bind(&global_sql)
    ...
    async fn delete_poisoned_cache(&self, school_id: &str, question_text: &str) -> Result<(), AppError> {
        let mut conn = self.client.acquire_tenant_connection(school_id).await?;
        sqlx::query("DELETE FROM ai_query_cache WHERE (school_id = $1 OR school_id = 'GLOBAL') AND question_text = $2")
            .bind(school_id)
            .bind(question_text)
            .execute(&mut *conn).await?;
        Ok(())
    ...
    async fn update_cache_hit_metrics(&self, school_id: &str, question_text: &str) -> Result<(), AppError> {
        let mut conn = self.client.acquire_tenant_connection(school_id).await?;
        sqlx::query(
            "UPDATE ai_query_cache SET search_count = search_count + 1, last_used_at = NOW() WHERE (school_id = $1 OR school_id = 'GLOBAL') AND question_text = $2"
        )
        .bind(school_id)
        .bind(question_text)
        .execute(&mut *conn).await?;
    ...
    async fn get_query_suggestions(&self, school_id: &str, query: &str, limit: i64) -> Result<Vec<Value>, AppError> {
        ...
        let mut ilike_pattern = String::from("%");
        for w in words {
            ilike_pattern.push_str(w);
            ilike_pattern.push_str("%");
        }

        let rows = sqlx::query(
            "SELECT question_text, search_count
             FROM ai_query_cache 
             WHERE (school_id = $1 OR school_id = 'GLOBAL')
        ...
        let mut res = Vec::new();
        for r in rows {
            let text: String = r.get("question_text");
            let count: i32 = r.get("search_count");
            res.push(json!({"text": text, "count": count, "similarity": 1.0}));
        }
        Ok(res)
    }
    ...
    async fn save_chat_history(&self, school_id: &str, session_id: &str, role: &str, content: &str) -> Result<(), AppError> {
        let mut conn = self.client.acquire_tenant_connection(school_id).await?;
        sqlx::query("INSERT INTO ai_chat_history (session_id, role, content) VALUES ($1, $2, $3)")
            .bind(session_id)
            .bind(role)
            .bind(content)
            .execute(&mut *conn).await?;
