...
impl SuperAdminRepository for PostgresSuperAdminRepository {
    async fn get_password_hash(&self, username: &str) -> Result<Option<String>, AppError> {
        ...
        let row = sqlx::query("SELECT password_hash FROM super_admins WHERE username = $1")
            .bind(username)
    ...
    async fn get_super_admin_profile(&self, username: &str) -> Result<Option<(String, Option<String>)>, AppError> {
        let mut conn = self.client.acquire_super_admin_connection().await?;
        let row = sqlx::query("SELECT username, profile_image_url FROM super_admins WHERE username = $1")
            .bind(username)
            .fetch_optional(&mut *conn)
            .await?;
        if let Some(r) = row {
    ...
    async fn update_super_admin(
        ...
        // Fetch old photo
        let row = sqlx::query("SELECT profile_image_url FROM super_admins WHERE username = $1")
        ...
        if current_username != new_username {
            sqlx::query("DELETE FROM super_admins WHERE username = $1")
                .bind(current_username)
                .execute(&mut *tx)
                .await?;
        }
        ...
        sqlx::query(
            "INSERT INTO super_admins (username, password_hash, profile_image_url) VALUES ($1, $2, $3)
