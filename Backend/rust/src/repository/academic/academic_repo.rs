...
impl crate::repository::traits::AcademicRepository for PostgresAcademicRepository {
    ...
    async fn get_exam_submissions(&self, school_id: &str, exam_id: &str, status: Option<&str>) -> Result<JsonList, AppError> {
        ...
        let rows = if let Some(s) = status {
            sqlx::query("SELECT ss.*, agr.overall_score, agr.grade, agr.feedback, agr.confidence_score, \
                 (SELECT COALESCE(json_agg(json_build_object('criterion_name', gc.criterion_name, 'score', gc.score, 'max_score', gc.max_score)), '[]'::json) FROM grade_criteria gc WHERE gc.grading_id = agr.grading_id) as criteria_scores
