...
fn map_school_fee(r: &sqlx::postgres::PgRow) -> Value {
    json!({
        "id": r.get::<String, _>("fee_id"),
        "feesName": r.get::<String, _>("name"),
        "feesReason": r.get::<String, _>("reason"),
        "feesPeriod": r.get::<String, _>("period"),
        "feesAmount": r.get::<f64, _>("amount"),
...
impl crate::repository::traits::FeeRepository for PostgresFeeRepository {
    async fn add_school_fee(&self, school_id: &str, data: Value) -> Result<Value, AppError> {
        ...
        sqlx::query("INSERT INTO fee_templates (fee_id, school_id, name, reason, period, amount) VALUES ($1, $2, $3, $4, $5, $6)")
            .bind(&fees_id)
            .bind(school_id)
    ...
    async fn get_school_fees(&self, school_id: &str) -> Result<JsonList, AppError> {
        let mut conn = self.client.acquire_tenant_connection(school_id).await?;
        let rows = sqlx::query(
            "SELECT fee_id, name, reason, period, amount::FLOAT as amount, created_at FROM fee_templates WHERE school_id = $1 ORDER BY created_at DESC",
        )
        .bind(school_id)
        .fetch_all(&mut *conn)
        .await?;
    ...
    async fn get_pending_fees(
        ...
        let query_str = if class_name.is_some() {
            "SELECT sf.student_id, sf.total_fees::FLOAT as total_fees, sf.pending_amount::FLOAT as pending_amount, \
             ...
             FROM student_invoices sf JOIN students s ON sf.student_id = s.student_id AND sf.school_id = s.school_id \
    ...
    async fn add_student_fee(
        ...
        let mut conn = self.client.acquire_tenant_connection(school_id).await?;
        sqlx::query("INSERT INTO student_invoices (student_id, school_id, fee_id, total_fees, pending_amount, status) VALUES ($1, $2, 'tuition', $3, $3, 'pending') ON CONFLICT (student_id) DO UPDATE SET total_fees = EXCLUDED.total_fees")
    ...
    async fn get_student_fee(
        ...
        let mut conn = self.client.acquire_tenant_connection(school_id).await?;
        let row =
            sqlx::query("SELECT student_id, total_fees::FLOAT as total_fees, pending_amount::FLOAT as pending_amount FROM student_invoices WHERE school_id = $1 AND student_id = $2")
                .bind(school_id)
                .bind(student_id)
    ...
    async fn update_student_fee(
        ...
        let mut conn = self.client.acquire_tenant_connection(school_id).await?;
        let exists: bool = sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM student_invoices WHERE school_id = $1 AND student_id = $2)")
            .bind(school_id)
            .bind(student_id)
            .fetch_one(&mut *conn)
            .await?;
        ...
        if exists {
            sqlx::query(
                "UPDATE student_invoices SET total_fees = COALESCE($1, total_fees), discount = COALESCE($2, discount), pending_amount = COALESCE($3, pending_amount) WHERE school_id = $4 AND student_id = $5",
            )
            ...
            sqlx::query(
                "INSERT INTO student_invoices (school_id, student_id, fee_id, total_fees, pending_amount, discount, status) 
