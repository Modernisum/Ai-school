    ...
    async fn add_student(
        ...
        sqlx::query(
            "INSERT INTO students (
                ...
                is_transport_enabled, transport_radius,
    ...
    async fn update_student(
        ...
        sqlx::query(
            "UPDATE students SET 
                ...
                is_transport_enabled = COALESCE($18, transport_enabled),
    ...
    async fn get_form_status(&self, school_id: &str) -> Result<JsonList, AppError> {
        ...
        let rows = sqlx::query(
            "SELECT s.student_id, s.name, s.class_name, s.created_at, s.updated_at, \
             ...
             (SELECT string_agg(file_url, ',') FROM document_boxes WHERE school_id = s.school_id AND user_id = s.student_id) as documents \
    ...
    async fn get_form_autofill_data(&self, school_id: &str, student_id: &str) -> Result<Option<Value>, AppError> {
        ...
        let row = sqlx::query(
            "SELECT name, class_name, data, dob, gender, father_name, mother_name, address_line1, aadhaar_number, \
             (SELECT string_agg(file_url, ',') FROM document_boxes WHERE school_id = $1 AND user_id = $2) as documents \
