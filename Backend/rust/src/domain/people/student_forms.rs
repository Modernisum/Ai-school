    ...
    match state.repos.student.get_form_autofill_data(&school_id, &student_id).await {
        Ok(Some(student)) => {
            ...
            if !docs_str.is_empty() {
                ...
                if let Ok(ai_client) = crate::grpc_client::AiClient::connect().await {
                    if let Ok(res) = ai_client.process_ocr(&school_id, doc_url, "aadhaar").await {
                        if res.success {
                            if let Ok(res_json) = serde_json::from_str::<serde_json::Value>(&res.extracted_fields_json) {
                                ocr_fields = res_json;
                            }
                        }
                    }
