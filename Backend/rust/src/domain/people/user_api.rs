...
pub async fn search_students_user(
    ...
    // Combine class_name and section if both provided to form the Space ID
    let space_id = if let (Some(cls), Some(sec)) = (&q.class_name, &q.section) {
        Some(format!("{}-{}", cls, sec))
    } else {
        q.class_name.as_deref().map(|s| s.to_string())
    };
