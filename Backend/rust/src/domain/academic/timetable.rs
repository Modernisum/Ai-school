...
pub async fn generate_timetable(
    ...
    match engine
        .generate_timetable(
            &school_id, &payload.class_id, &payload.class_name,
            periods, days, payload.requirements,
            payload.season, start_time, end_time,
            period_dur, break_dur,
        )
        .await
    {
        Ok(result) => Json(json!({
            "success": true, "config_id": result.config_id,
            "class_id": result.class_id, "class_name": result.class_name,
            "total_slots": result.slots.len(), "slots": result.slots,
            "conflicts": result.conflicts, "has_conflicts": !result.conflicts.is_empty(),
        })).into_response(),
        Err(e) => (
